...
# b.attach_kprobe(event=execve_fnname, fn_name="syscall__io_schedule")
# b.attach_kretprobe(event=execve_fnname, fn_name="do_ret_sys_io_schedule")
...
if args.print_uid:
    print("%-6s" % ("UID"), end="")
print("%-16s %-6s %-6s %-3s %16s" % ("PCOMM", "PID", "PPID", "RET",  "SCHEDULED OUT DUR(ns)"))
...
def print_event(cpu, data, size):
    ...
    if event.type == EventType.EVENT_RET:
        ...
        if not skip:
            ...
            printb(b"%-16s %-6d %-6s %-3d %16d" % (event.comm, event.pid,
